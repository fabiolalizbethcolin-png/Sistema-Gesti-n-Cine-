...
<div class="auth-container">
    ...
    <form action="{{ route('funciones.store') }}" method="POST">
        ...
        <div>
            ...
            <select name="pelicula_id">
                ...
                @foreach($peliculas as $pelicula)
                    <option value="{{ $pelicula->id }}" {{ (session('old')['pelicula_id'] ?? '') == $pelicula->id ? 'selected' : '' }}>
                        {{ $pelicula->titulo }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            <label>Sala:</label>
            <input type="text" name="sala" value="{{ session('old')['sala'] ?? '' }}">
        ...
        <div>
            <label>Fecha:</label>
            <input type="date" name="fecha" value="{{ session('old')['fecha'] ?? '' }}">
        ...
        <div>
            <label>Hora:</label>
            <input type="time" name="hora" value="{{ session('old')['hora'] ?? '' }}">
        ...
        <div>
            <label>Precio:</label>
            <input type="number" name="precio" value="{{ session('old')['precio'] ?? '' }}">
            @if(session('errors')['precio'] ?? false)
                <small style="color:red;">{{ session('errors')['precio'] }}</small>
            @endif
        </div>

        <button type="submit" style="margin-top:15px;">Agregar Función</button>
    </form>
</div>
@endsection
